\
// App JS with fixes for Github Pages and import/export behavior
let planEstudios = {};
let cursosHorarios = {};
...
function horaAMinutos(horaStr){ const [h,m] = horaStr.split(":").map(x=>parseInt(x,10)); return h*60 + m; }
function horariosSeCruzan(horarios1, horarios2){
  for(const h1 of horarios1){ for(const h2 of horarios2){
    if(h1.día === h2.día){
      const h1_inicio = horaAMinutos(h1.inicio); const h1_fin = horaAMinutos(h1.fin);
      const h2_inicio = horaAMinutos(h2.inicio); const h2_fin = horaAMinutos(h2.fin);
      if(!(h1_fin <= h2_inicio || h2_fin <= h1_inicio)) return true;
    }
  }} return false;
}
function cartesian(arrays){ if(arrays.length===0) return [[]]; const [first,...rest]=arrays; const restProd=cartesian(rest); const res=[]; for(const f of first) for(const r of restProd) res.push([f,...r]); return res; }
...
// fetch plan with fallback (if fetch fails, show uploader)
...
function renderSemestres(){
  ...
  if(keys.length===0){
    container.innerHTML = '<div class="text-sm text-gray-500">No hay semestres cargados. Puedes subir un archivo <strong>plan_estudios.json</strong>.</div>';
    return;
  }
  keys.forEach(sem=>{
  ...
  document.querySelectorAll('.sem-checkbox').forEach(cb=> cb.addEventListener('change', renderCursosDisponibles));
...
document.getElementById('btn-add-cursos').addEventListener('click', ()=>{
  document.querySelectorAll('.curso-checkbox:checked').forEach(cb=>{
    const codigo = cb.dataset.codigo; const nombre = cb.dataset.nombre;
...
document.getElementById('btn-load-sample').addEventListener('click', ()=>{
  const sems = Object.keys(planEstudios); if(sems.length>0){
    const cursos = planEstudios[sems[0]]; let i=0;
    for(const cod of Object.keys(cursos)){ cursosSeleccionados[cod]={nombre:cursos[cod].nombre, creditos:cursos[cod].creditos}; cursosHorarios[cod]={}; i++; if(i>=3) break; }
    renderConfigCursos();
  }
});
...
function renderConfigCursos(){
  ...
  Object.keys(cursosSeleccionados).forEach(codigo=>{
    ...
    card.innerHTML = `<div class="flex justify-between items-center"><div><strong>${curso.nombre}</strong> <div class="text-xs text-gray-500">${codigo}</div></div><div class="flex gap-2"><button class="btn-primary text-xs" data-action="add-group" data-codigo="${codigo}">+ Grupo</button><button class="btn-primary text-xs" data-action="remove-curso" data-codigo="${codigo}">Eliminar</button></div></div><div class="groups mt-2" id="groups_${codigo}"></div>`;
  ...
  document.querySelectorAll('[data-action="remove-curso"]').forEach(btn=>btn.addEventListener('click', ()=>{ delete cursosSeleccionados[btn.dataset.codigo]; delete cursosHorarios[btn.dataset.codigo]; renderConfigCursos(); }));
}
...
function renderGroups(codigo){
  ...
  Object.keys(groups).forEach(gk=>{
    const g = groups[gk];
    const gDiv = document.createElement('div'); gDiv.className='p-2 bg-white rounded-md border mb-2';
...
document.getElementById('btn-generar').addEventListener('click', ()=>{ const valid = combinacionesValidas(); combinacionesGeneradas = valid; combinacionesVars = new Array(valid.length).fill(false); document.getElementById('combinaciones-info').textContent = `Se encontraron ${valid.length} combinaciones válidas.`; const list = document.getElementById('combinaciones-list'); list.innerHTML=''; valid.forEach((comb,idx)=>{ const div = document.createElement('div'); div.className='flex items-center justify-between p-2 bg-white rounded'; div.innerHTML = `<label class="flex items-center gap-2"><input type="checkbox" data-idx="${idx}" class="comb-cb"> Combinación ${idx+1} <span class="text-xs text-gray-400">(${comb.length} grupos)</span></label><div class="flex gap-2"><button class="btn-primary btn-ver" data-idx="${idx}">Ver</button></div>`; list.appendChild(div); }); document.querySelectorAll('.comb-cb').forEach(cb=>cb.addEventListener('change', (e)=>{ combinacionesVars[parseInt(cb.dataset.idx,10)] = cb.checked; })); document.querySelectorAll('.btn-ver').forEach(b=>b.addEventListener('click', ()=> previewCombinacion(parseInt(b.dataset.idx,10)))); });
...
function drawHorario(combinacion){
  const area = document.getElementById('schedule-canvas'); area.innerHTML='';
  const slotHeight = 36;
  ...
  combinacion.forEach((g,idx)=>{
    ...
    g.horarios.forEach(h=>{
      ...
      let top=0, height=0;
      if(startIdx!==-1 && endIdx!==-1){ top = startIdx * slotHeight + 28; height = (endIdx - startIdx + 1) * slotHeight - 6; }
      else { const start = horaAMinutos(h.inicio); const end = horaAMinutos(h.fin); const base = horaAMinutos(timeSlots[0][0]); top = ((start-base)/60)*slotHeight + 28; height = ((end-start)/60)*slotHeight - 6; }
      const dayCols = area.querySelectorAll('.day-column'); const col = dayCols[dayIndex];
      const block = document.createElement('div'); block.className='course-block'; block.style.background=color; block.style.position='absolute'; block.style.left='8px'; block.style.right='8px'; block.style.top=top+'px'; block.style.height=Math.max(20,height)+'px'; block.style.zIndex=10; block.style.display='flex'; block.style.flexDirection='column'; block.style.justifyContent='center'; block.style.padding='6px'; block.style.boxSizing='border-box';
  ...
  // ensure legend visible background for canvas capture
  legend.querySelectorAll('div').forEach(d=> d.style.background='#ffffff');
}

// export/import and init
...
// plan upload fallback
document.getElementById('btn-upload-plan').addEventListener('click', ()=> document.getElementById('file-plan').click());
document.getElementById('file-plan').addEventListener('change', async e=>{ const f=e.target.files[0]; if(!f) return; const txt=await f.text(); try{ planEstudios = JSON.parse(txt); document.getElementById('plan-upload').classList.add('hidden'); renderSemestres(); alert('Plan cargado.'); }catch(err){ alert('Plan inválido.'); } });
...
document.getElementById('btn-export-pdf').addEventListener('click', async ()=>{ const selectedIdx = combinacionesVars.map((v,i)=> v? i : -1).filter(i=>i!==-1); if(selectedIdx.length===0){ alert('Selecciona al menos una combinación para exportar.'); return; } const { jsPDF } = window.jspdf; const pdf = new jsPDF('landscape','pt','a4'); for(let k=0;k<selectedIdx.length;k++){ drawHorario(combinacionesGeneradas[selectedIdx[k]]); await new Promise(r=>setTimeout(r,250)); const canvas = await html2canvas(document.getElementById('schedule-canvas'), {useCORS:true, backgroundColor:'#ffffff', scale:1.5}); const img = canvas.toDataURL('image/png'); const imgProps = pdf.getImageProperties(img); const pdfW = pdf.internal.pageSize.getWidth(); const pdfH = (imgProps.height * pdfW) / imgProps.width; if(k>0) pdf.addPage(); pdf.addImage(img,'PNG',0,0,pdfW,pdfH); } pdf.save('horarios.pdf'); });
